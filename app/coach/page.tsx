...
export default function CoachPage() {
  const partner = getUser(exchange.b);
  const doneCount = exchange.tasks.filter((t) => t.status === "done").length;
  const progress = Math.round((doneCount / exchange.tasks.length) * 100);
  const todayTasks = exchange.tasks.filter((t) => t.status !== "done");
  const week = exchange.plan[0];
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      ...
      <div className="glass mt-6 rounded-3xl p-6">
        ...
        <p className="text-gradient mt-2 text-lg font-semibold">{exchange.goal}</p>
